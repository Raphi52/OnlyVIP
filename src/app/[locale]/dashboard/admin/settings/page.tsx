          ...
          >
            <Card variant="luxury" className="p-4 sm:p-6">
              ...
              <div className="space-y-4">
                ...
                {/* Platform Wallets - Read Only (from env vars) */}
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                  <div className="flex items-center gap-2 mb-3">
                    <Shield className="w-5 h-5 text-emerald-400" />
                    <span className="font-medium text-emerald-400">Secure Wallet Configuration</span>
                  </div>
                  <p className="text-sm text-[var(--muted)] mb-4">
                    For security, wallet addresses are configured via environment variables on the server.
                    They cannot be changed from this interface to prevent unauthorized modifications.
                  </p>

                  {/* Show current wallets (read-only) */}
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-medium text-[var(--muted)] mb-1">
                        PayGate Wallet (Polygon/USDC)
                      </label>
                      <div className="px-4 py-2 rounded-lg bg-[var(--background)] border border-[var(--border)] font-mono text-sm text-[var(--foreground)] opacity-70">
                        {settings.platformWalletEth ?
                          `${settings.platformWalletEth.slice(0, 10)}...${settings.platformWalletEth.slice(-8)}` :
                          "Not configured"}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--muted)] mb-1">
                        NOWPayments Wallet (BTC)
                      </label>
                      <div className="px-4 py-2 rounded-lg bg-[var(--background)] border border-[var(--border)] font-mono text-sm text-[var(--foreground)] opacity-70">
                        {settings.platformWalletBtc ?
                          `${settings.platformWalletBtc.slice(0, 10)}...${settings.platformWalletBtc.slice(-8)}` :
                  ...
                  <p className="text-xs text-[var(--muted)] mt-3">
                    To change wallets, update PAYGATE_WALLET_ADDRESS in your server environment and restart.
                  </p>
                </div>
